...
public class Assignment implements TaskComparatorInterface, DebugInterface {
	 ...
	 * }
	 * 
	 * The jist of this is to use the interface and not the implementation.
	 * So if you all of a sudden realize you prize insertion performance over
	 *  random access, its as simple a change as doing this:
	 *  
	 ...
	 *  
	 *	Assignment
     *   + Create a github repository to hold homework named bma-week-7
     *      containing a class, Assignment, in the default package
     *   + Take your Task hierachy from week 6 and place into one or more packages
     *       + Create a base package for your application
     *           + Including a unique prefix is optional (com.crossfireconsulting
     *              for example)
     *           +  The application prefix should be present.  Tasklist, todolist,
     *               todo, whatever...
     *       + Its time we move to one class per file, so make each file live in its
     *          own class (anonymous and inner classes are obviously exceptions)
	 *   + Implement a package called TaskFilter off of the application root
	 *      package that will filter tasks 
     *       + Implement the TaskFilter interface.  The most simple interface I
     *          can think of would be "public Task[] filter(Task[] tasks);"
     *       + 3.2) Implement at least one class that implements TaskFilter
     *           hint: you can bring in as much or as little data and behavior
     *            as long as you implement the public interface
     *           hint: remember this involves by reference semantics.  You
     *            don't want to destroy the original collection of tasks
     *   In Assignment.java
     ...
     *       + Filter the collection of Task objects and print the resulting
     *          collection of Task objects
     *       
     *   Git:
     *   Starting with this HW, I duplicated the previous week's HW repo to
     *    retain prior commits.
	...
	private static void week7Main() {
		...
		// TaskFilter sorts by default when Full Featured constructor is used
		// and specifies sort parameters
		System.out.println( "TaskFilter[] Output\n(Anonymous output is seen"
					+ "in first Array element (sort by Task name)\n without "
					+ "TaskFilter member fields: filterField and sortOrder):\n"
	 ...
	 *        
	 *    Assignment
	 *        + Create a github repository to hold homework named bma-week-6
	 *           containing a class, Assignment
	 *        + Copy the Task class you created last week into the
	 *           Assignment.java file
	 *           + The previous two tasks could be achieved by cloning your
	 *              week 5 repository
	 *        + Apply access modifiers to your Task class
	 *            + All instances variables should be made private
	 *            + Determine what functionality and data you want to make
	 *               accessible to outside callers via methods
	 *                Some examples could include the following but I'm not
	 *                  trying to design your solutions.  These are just ideas
	 *                    isComplete(): bool
	 ...
	 *            + Consider different kinds of tasks (todo items) your todo
	 *               list could have. 
	 ...
	 *                + Hide all instance variables with private modifiers if
	 *                   you didn't already
	 *                	xxx I used protected, otherwise, inheritance wouldn't work
	 *                  * Class member fields are now privatized and are accessed
	 *                     via getters and setters
	 *                + Implement the common functionality from above as methods
	 *                + Add accessors (getters and/or setters) for any member
	 *                   variables you think need them
	 *                    You don't necessarily need both
	 *                    Its easier to add one later if you need it than to
	 *                     take one away
	 ...
	 *            + In the Assignment class, create a method "private static
	 *               void printTask(Task)" that prints out Task objects
	 *            + In Assignment's main() method, create at least one of each
	 *               type of Task created earlier.
	 *            + Send each task to printTask()
	 *            + [bonus]: test the rest of your Task parent class' public interface.
	 *
	 *	This assignment is aimed at both giving a feeling for inheritance and
	 *   for getting you thinking about implementation ideas for your project.
	 *   It might be that your solution doesn't have a need to inheritance
	 *   hierarchies.  That's fine but there are two goals here.
	 *
	 */
	...
	private static void week6Main() {
		...
		HomeworkTask homeworkTask1
			= new HomeworkTask("New Homework Task 1"
								, (System.currentTimeMillis() + 1));
		
		HomeworkTask homeworkTask2
			= new HomeworkTask("New Homework Task 2"
	 ...
	 *        + Create a github repository to hold homework.  It should be named
	 *           bma-week-5.  It should contain an Assignment.java file
	 ...
	 *                In the future we'll try to avoid this, but we currently
	 *                 don't have error handling in our toolbox
	 *            + Otherwise, return the array element at the index
	 *        + Implement a class named Task inside the Assignment.java file
	 *            + Do not create a new file.  Place the Task class immediately
	 *               after the end of the Assignment class in Assignment.java
	 ...
	 *                + Call every constructor implemented in Task at least once,
	 ...
	 *                Why didn't it work?  The compiler has no idea how to
	 *                 convert a Task object into a String
	 *            + Implement a toString() method in Task
	 *                + The signature has to be "public String toString()" exactly
	 *                + Create a String representation of a Task.  This does
	 *                   not need to display every piece of data in the class,
	 *                   just the core information.  "Task(title)" would be a
	 *                   sparse but acceptable implementation.
	 *            + Try to run Assignment again
	 *                + It should work.  If not, try debugging.  If that fails,
	 *                   send an email
	 *
	 */
	...
	private static void week5Main() {
		...
		// Instantiate new task objects
		Task task1 = new GeneralTask((myTaskName + "1")
				, (System.currentTimeMillis() + 1));
		Task task2 = new GeneralTask((myTaskName + "2")
				, (System.currentTimeMillis() + 2));
	...
	private static void dumpList(List<TaskFilter> tskFltrLst) {
		...
		while ( taskFilterIterator.hasNext() ) {
			TaskFilter tf = taskFilterIterator.next();
			
			// TODO Why doesn't this work???
			// System.out.println(tf.toString());
			
			System.out.println("\nSort Field: " + tf.getSortFieldName()
								+ " (" + tf.getSortField() + ")"
								+ "\nSort Order: " + tf.getSortOrderName());
			
			Iterator<Task> taskListIterator = tf.getList().iterator();
			while ( taskListIterator.hasNext() ) {
				System.out.println(taskListIterator.next().toString()
									+ "\n\n-------------------");
			}
			System.out.println("\n============================");
		}
